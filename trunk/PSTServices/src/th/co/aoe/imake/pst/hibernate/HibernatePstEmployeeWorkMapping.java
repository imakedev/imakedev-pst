...
public class HibernatePstEmployeeWorkMapping  extends HibernateCommon implements PstEmployeeWorkMappingService {
	...
	private int getSize(Session session, PstEmployeeWorkMapping instance) throws Exception{
		try {
			...
			StringBuffer sb =new StringBuffer(" select count(pstEmployeeWorkMapping) from PstEmployeeWorkMapping pstEmployeeWorkMapping ");
	 ...
	 public List searchPstEmployeeWorkMapping(PstEmployeeWorkMapping instance,Pagging pagging) throws DataAccessException {
			...
			try {
				...
				StringBuffer sb =new StringBuffer(" select pstEmployeeWorkMapping from PstEmployeeWorkMapping pstEmployeeWorkMapping ");
				...
				if(pagging.getSortBy()!=null && pagging.getSortBy().length()>0){
						sb.append( " order by pstEmployeeWorkMapping."+pagging.getOrderBy()+" "+pagging.getSortBy().toLowerCase());
				}			
				 query =session.createQuery(sb.toString());
				// set pagging.
